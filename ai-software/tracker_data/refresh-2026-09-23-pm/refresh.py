"""2026-09-23 post-close AI Software refresh.

Run from the repository root:
  env -u PYTHONPATH uv run --isolated --python 3.11 --with yfinance --with pandas \
    python ai-software/tracker_data/refresh-2026-09-23-pm/refresh.py
...
def fill_verified_gap(frame: pd.DataFrame, ticker: str) -> pd.DataFrame:
    """Backfill Yahoo-null Sep 22 stocks from Nasdaq daily OHLCV evidence."""
    if ticker not in GAP_SOURCES:
        assert GAP_DAY in frame.index, (ticker, "unexpectedly missing Sep 22")
        return frame
    record = GAP_SOURCES[ticker]
    assert record["date"] == "2026-09-22"
    assert record["provider"] == "Nasdaq historical daily stock OHLCV"
    assert record["http_status"] == 200 and record["source_url"].startswith("https://api.nasdaq.com/")
    close = float(record["close"])
    if GAP_DAY in frame.index:
        assert abs(float(frame.loc[GAP_DAY, "Close"]) - close) < 0.03, ticker
        return frame
    for text_day, expected in record["adjacent_days"].items():
        day = pd.Timestamp(datetime.strptime(text_day, "%m/%d/%Y").date())
        assert abs(float(frame.loc[day, "Close"]) - float(expected.lstrip("$"))) < 0.03, (ticker, day)
    ...
    frame = pd.concat([frame, pd.DataFrame([row], index=pd.DatetimeIndex([GAP_DAY], name="Date"))]).sort_index()
    assert not frame.index.duplicated().any(), ticker
    print("verified Nasdaq daily gap", ticker, str(GAP_DAY.date()), close, record["source_url"])
    return frame
...
def main() -> None:
    ...
    evidence = {
        "provider": "Yahoo Finance / yfinance auto_adjust=True; dated Sep 22 null daily bars from Nasdaq historical OHLCV",
